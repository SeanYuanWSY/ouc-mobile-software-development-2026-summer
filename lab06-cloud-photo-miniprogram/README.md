...
- 仅本人照片可调用；固定服务地址与文件路径、图片最多 10 MiB、响应最多 64 KiB、40 秒请求预算、无自动重试；每分钟 3 次、每日 20 次尝试（日界为 UTC）。客户端额外间隔 60 秒。
- 独立函数 `lab06_aiReview` 已部署，执行时限 60 秒；独立集合 `lab06_ai_usage` 已创建为 ADMINONLY，不修改其他项目的 AI 配置。
