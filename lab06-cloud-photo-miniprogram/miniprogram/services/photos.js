...
async function list(owner, offset = 0) {
 ...
 return data.map(p => Object.assign({},p,{ initial: (p.nickName || "摄").slice(0,1), dateLabel: stamp(p.createdAt) }))
}
async function detail(id) {
 ...
 return Object.assign({},data,{initial:(data.nickName || "摄").slice(0,1),dateLabel:stamp(data.createdAt)})
}
async function commit(record, openid) {
 try { await collection().add({ data: Object.assign({}, record, {createdAt:wx.cloud.database().serverDate()}) }) }
  ...
  let existing
  try { existing = await detail(record._id) } catch (_) { /* Keep pending work when confirmation is unavailable. */ }
...
async function publish(file, input) {
 ...
 const { fileID } = await wx.cloud.uploadFile({ cloudPath: 'lab06/photos/' + id + '.' + ext, filePath: file.tempFilePath })
