...
let sessionKey='', inFlight=false, nextAllowed=0
...
async function generate(photoId,style){
 ...
 if(Date.now()<nextAllowed)throw new Error('请等待 '+Math.ceil((nextAllowed-Date.now())/1000)+' 秒后重试；上一请求可能已发送')
 ...
 if(inFlight||Date.now()<nextAllowed)throw new Error('请等待 '+Math.ceil((nextAllowed-Date.now())/1000)+' 秒后重试；上一请求可能已发送')
 inFlight=true;nextAllowed=Date.now()+60000
 let result
 try{
 ...
 }catch(_){throw new Error('暂时无法获取点评，请稍后重试；请求可能已发送，请勿连续点击')}
 finally{inFlight=false}
 const errors={PHOTO:'暂时无法读取照片，请刷新后重试',QUOTA:'用量校验暂时失败，本次未发送给 DeepSeek，请重试',IMAGE:'图片读取失败，本次未发送给 DeepSeek，请刷新照片后重试',PROVIDER_AUTH:'DeepSeek 拒绝了 API Key，请检查是否填写正确',PROVIDER_BALANCE:'DeepSeek 账户余额不足，请检查账户额度',PROVIDER_RATE:'DeepSeek 当前请求较多，请稍后重试',MODEL:'所选模型不支持图片，请选择 Vision 视觉模型',KEY:'API Key 格式有误，请重新填写',AUTH:'请先登录微信',OWNER:'只能点评自己发布的照片',INPUT:'照片或风格无效',LIMIT:'请求过于频繁，或本日已达 20 次上限'}
 if(result&&!result.ok&&['KEY','AUTH','OWNER','INPUT','MODEL','PHOTO','QUOTA','IMAGE'].includes(result.code))nextAllowed=0
