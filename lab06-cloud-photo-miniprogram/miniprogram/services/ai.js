...
async function generate(photoId,style){
 ...
 if(Date.now()<nextAllowed)throw new Error('请稍候再生成，避免重复请求与计费')
 ...
 if(inFlight||Date.now()<nextAllowed)throw new Error('请稍候再生成，避免重复请求与计费')
 ...
 const errors={MODEL:'所选模型不支持图片，请选择 Vision 视觉模型',KEY:'API Key 格式有误，请重新填写',AUTH:'请先登录微信',OWNER:'只能点评自己发布的照片',INPUT:'照片或风格无效',LIMIT:'请求过于频繁，或本日已达 20 次上限'}
 if(!result||!result.ok)throw new Error(errors[result&&result.code]||'AI 服务暂不可用，请检查 Key、账户额度或稍后重试')
