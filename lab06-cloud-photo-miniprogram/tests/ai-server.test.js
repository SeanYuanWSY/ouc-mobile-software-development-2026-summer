...
function harness(opts={}){
 ...
 const cloud={init(){},getWXContext:()=>({OPENID:opts.noAuth?'':owner}),database:()=>db,getTempFileURL:async()=>({fileList:[{status:0,tempFileURL:opts.badUrl||'https://636c-cloudbase-d5gdro8i30f1a4efd-1481960851.tcb.qcloud.la/lab06/photos/test_owner/p_test.jpg?sign=test'}]})}
...
for(const [name,options,changes] of [['no auth',{noAuth:true},{}],['foreign photo',{foreign:true},{}],['text model',{}, {model:'deepseek-v4-pro'}],['quota unavailable',{dbFail:true},{}],['invalid key',{}, {apiKey:'bad'}]])test(name+' rejects before external request',async()=>{const h=harness(options);const r=await h.run(changes);assert.equal(r.ok,false);assert.equal(h.stats().calls,0);assert.ok(!JSON.stringify(r).includes('SECRET'))})
test('first quota record and transaction retry issue only one provider request',async()=>{const h=harness({retry:true});assert.equal((await h.run()).ok,true);assert.equal(h.stats().calls,2);assert.equal(h.stats().row.count,1);assert.equal(h.stats().writes,1)})
test('daily and minute limits reject before external calls',async()=>{for(const row of [{day:Math.floor(Date.now()/86400000),count:20},{day:Math.floor(Date.now()/86400000),count:1,minute:Math.floor(Date.now()/60000),recent:3}]){const h=harness({row});assert.equal((await h.run()).code,'LIMIT');assert.equal(h.stats().calls,0)}})
