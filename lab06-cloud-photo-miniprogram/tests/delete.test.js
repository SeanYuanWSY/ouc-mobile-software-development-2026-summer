...
function harness(opts={}){
 ...
 prepareSubject({subject:'../cloudfunctions/lab06_deletePhoto/index',mocks:[{spec:'wx-server-sdk',value:cloud}]})
 return{run:args=>require('../cloudfunctions/lab06_deletePhoto/index').main(args||{photoId:'p_test'}),row,events,opts,calls:()=>calls}
}
...
test('tombstone inaccessible to signing and AI but accepted for cleanup',()=>{const h=harness();h.row.deleted=true;assert.equal(require('../cloudfunctions/lab06_deletePhoto/policy').allowed(h.row),true);assert.equal(require('../cloudfunctions/lab06_photoAccess/policy').allowed(h.row),false);assert.equal(require('../cloudfunctions/lab06_aiReview/policy').allowed(h.row),false)})
