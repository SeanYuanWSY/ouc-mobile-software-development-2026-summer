...
- 支持二次确认后退出本机资料；以身份删除作为退出提交点，提交后的资料互动或头像清理失败时会在下次启动重试；
- 支持从足迹列表重新进入新闻详情；
- 提供沉浸式新闻流的快捷入口。
...
新闻标题、日期和图片取自中国海洋大学新闻网公开报道，每条数据均保留原文 URL；正文在项目中整理为适合课程演示的简短摘要，阅读时间明确标记为估算值。游客与已保存资料使用彼此独立的本地存储命名空间：游客状态的收藏和点赞会暂存在本机，首次保存资料后资料足迹必定从 0 开始，旧游客数据即使清理遇到瞬时失败也不会计入资料；从已完成归属清理的 v1 版本升级时，会幂等保留当时已经属于资料的有效互动。项目没有虚构全网点赞数，也没有把本地资料包装成微信账号认证。微信不允许小程序静默读取头像和昵称，因此必须由用户在原生控件中主动选择或确认；当前项目没有后端、OpenID 会话或跨设备同步：
...
> 真实性边界：`chooseAvatar` 与 `input type="nickname"` 是微信官方头像昵称填写能力，不是静态贴图；它们允许用户复用微信头像和昵称，也允许用户自行选择或编辑。真正的微信账号登录必须增加服务端，通过 `wx.login` 返回的临时 code 调用 `code2Session` 换取 OpenID。本实验没有服务端，因此界面只称“保存本机资料”，不声称已经完成账号认证。
