...
2026-09-17，资料删除保护已更新到dataService、assistantInbox和assistantGateway；云端18个源码/配置文件下载回读一致，公众平台确认00:00:56上传的1.1.1为体验版，仍无审核或线上版本。删除后迟到的旧保存请求会被拒绝，已删除资料不能再新建接力；本地MCP区分鉴权、权限、超时、限流与响应错误。电脑接力公网入口仍关闭，真实电脑客户端尚未接通。管理员直调需要的宽权限不是普通用户接入方案，已撤回旧文档的推荐。
