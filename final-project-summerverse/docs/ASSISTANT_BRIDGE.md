...
## 当前体验版：1.1.5（本地界面与云MCP协议准备，公网接力待验收）
...
【未验收】腾讯云`/assistant`路由仍关闭，尚未用真实Kimi HTTP MCP客户端完成领取、回传和撤销闭环。开启前仍需确认路由鉴权、限频、日志和套餐能力；Kimi Claw当前不能直接作为自定义MCP客户端，演示优先使用Kimi Code/支持HTTP MCP的客户端。
