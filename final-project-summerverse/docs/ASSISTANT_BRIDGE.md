...
## 当前体验版：1.1.5（公网路由已开启，体验版待重新上传）

【已核实】本轮把首页、资料工作台和电脑接力页统一为“一个主操作、按需展开”的中文界面；资料导入、AI整理、接力和原有数据逻辑保持不变。通过微信开发者工具编译检查，项目本地225项测试通过。

【已核实】`assistantGateway`新增无状态JSON-RPC MCP入口，复用个人连接令牌和原有`library.*`、`job.*`权限；新增握手、工具列表、工具调用和伪造字段测试。它只是云函数代码准备，不代表公网路由已开启。

【已核实】腾讯云默认域名上的`/assistant`路由已开启并绑定`assistantGateway`。匿名POST请求返回稳定的`401 UNAUTHORIZED`，未携带个人连接令牌时不能读取任务；路由本身未开启平台资源限频，控制台将该能力标为标准版，因此当前只适合本人演示，不应直接作为公开服务。尚未用真实Kimi HTTP MCP客户端完成领取、回传和撤销闭环；Kimi Claw当前不能直接作为自定义MCP客户端，演示优先使用Kimi Code/支持HTTP MCP的客户端。

【已核实】删除资料可靠性修复已增量部署至dataService、assistantInbox及assistantGateway，18个源码/配置文件下载比对一致，1.1.1体验版00:00:56上传并由公众平台回读确认。已删除资料无法新建接力；既有接力快照独立保留，需另行取消或清除。证据见`ACCEPTANCE_STATUS.md`当前记录。

【已核实】本地MCP增加安全错误分类；撤回旧`tcb:*`管理员直调权限推荐，真实客户端尚未接入。2026-09-17页面显示个人版、HTTP网关总开关打开但`/assistant`仍禁用，限频规则为空，资源限频标注标准版。未读取或转移凭据、开放入口或新增付费。接入配置以`integrations/mcp/README.md`为准，不能将云函数已部署视为电脑接力已完成。
...
【已核实，本地】`tests/assistant-jobs.test.js`包含实际启动 `server.cjs` stdio子进程并走业务服务的隔离闭环；使用合成数据库和测试网络响应。`tests/relay-client.test.js`验证发送确认、版本/来源白名单、回执丢失重试、重连保持同一请求、换连接拒绝与离页晚到结果。安全复核中的历史领取标识复用、已采纳任务孤立留存和重连重复派单均已修复。它们不证明真实Kimi客户端已完成云端领取/回传闭环；公网路由匿名拒绝已在控制台外部请求中核实。
...
4. 当前`/assistant`已按个人演示需求开启，但平台层资源限频仍未配置，路由开启不是内部配额的替代，也不是费用封顶承诺。公开给其他用户前必须补平台限频、日志脱敏核验和真实外部客户端闭环。
