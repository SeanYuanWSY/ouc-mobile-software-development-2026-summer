...
【已核实】本轮把首页、资料工作台和电脑接力页统一为“一个主操作、按需展开”的中文界面；体验模式名称调整为“工作台 / 记录”，资料导入、AI整理、接力和原有数据逻辑保持不变。通过微信开发者工具编译检查，项目本地225项测试通过。

【已核实】体验版1.1.6已上传成功，包含首页移除“专注”常驻模式、资料页移除重复专注按钮、首页与SummerTwin品牌标题统一样式；上传包大小约1.57 MB。
