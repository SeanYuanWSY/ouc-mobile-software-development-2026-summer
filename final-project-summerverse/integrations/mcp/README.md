...
**此模式仅保留为开发者诊断入口，暂不作为普通用户接入方案。** 2026-09-16复核撤回此前“环境内 `tcb:*` 是最小权限、保管得当即可接受”的建议：它包含整个环境的管理权限，持有人可绕过应用连接凭证的范围限制。不要向普通用户或电脑AI客户端发放这类凭据，不因 `SIGN_PARAM_INVALID` 自动扩权。

替代方案也不能只看接口名：腾讯云[现行CAM权限表](https://cloud.tencent.com/document/product/598/70005)将 SCF `InvokeFunction` 列为操作级权限，不能据旧文档宣称已限制到单函数；`InvokeFunctionUrl` 的资源级权限仍需验证CloudBase实际支持、入口防护与费用。当前没有已验证的最小权限管理员直调方案。

面向用户仍优先使用模式一的受保护HTTPS入口和每用户连接凭证；需先完成入口限流、预算与认证验收。任何CAM密钥都只属于管理员私有配置，不提交Git、不发给模型。本文不授权读取或迁移现有凭据。
...
## 手机派任务给电脑（1.1.0体验版，本机匹配可用）

2026-09-12已部署配套云函数并确认新版体验版。当前推荐个人演示使用本机匹配：手机创建一次性凭证，电脑端使用本地适配器连接已登录的微信开发者工具，再由 Kimi Code 读取和回传任务。公网路由仍关闭，因此这不是面向陌生用户的公开接入。
...
删除资料库原文不会撤回已经明确发送的独立接力快照；要停止该接力，请在接力列表取消或清除。已删除资料不能再创建新的接力任务。
