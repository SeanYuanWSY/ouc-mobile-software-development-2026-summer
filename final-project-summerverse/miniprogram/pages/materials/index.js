...
Page(withExperience({
  data: { workspace: null, sourceViews: [], resultViews: [], modes: MODES, selectedMode: 'requirements', question: '', busy: false, progress: '', error: '', notice: '', saveState: '', unsaved: false, storageMode: '',
    textOpen: false, textName: '', textDraft: '', linkOpen: false, linkDraft: '', sessions: [], libraryOpen: false, evidenceOpen: null, completed: 0, modeInfo: MODES[0], showAllModes: false, showImportOptions: false, showModePicker: false },
  onLoad(options = {}) {
  ...
  async newWorkspace() {
    ...
    this.setData({ error: '', notice: '', saveState: '', question: '', libraryOpen: false, textOpen: false, linkOpen: false, linkDraft: '', showImportOptions: false, showModePicker: false });
  },
  ...
  async restoreWorkspace(id) {
    await this.run(async (guard) => {
      ...
      this.setData({ libraryOpen: false, question: workspace.analysis?.question || '', selectedMode: mode.id, modeInfo: mode, showAllModes: MODES.indexOf(mode) > 3, showImportOptions: false, showModePicker: false, saveState: '已恢复资料', textOpen: false, linkOpen: false, linkDraft: '', evidenceOpen: null });
    });
  },
  ...
  toggleModePicker() { if (!this.data.busy) this.setData({ showModePicker: !this.data.showModePicker }); },
  ...
  selectMode(event) { const mode = MODES.find((m) => m.id === event.currentTarget.dataset.mode); if (!this.data.busy && mode) this.setData({ selectedMode: mode.id, modeInfo: mode, showModePicker: false }); },
